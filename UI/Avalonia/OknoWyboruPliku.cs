...
class OknoWyboruPliku
{
	public static string? OtworzJeden(string tytul, string? opis = null, string? maska = null, string? katalog = null)
	{
		...
		if (!String.IsNullOrEmpty(katalog)) opts.SuggestedStartLocation = TUI.StorageProvider.TryGetFolderFromPathAsync(new Uri("file://" + katalog)).GetAwaiter().GetResult();
		var pliki = TUI.StorageProvider.OpenFilePickerAsync(opts).GetAwaiter().GetResult();
	...
	public static string? Zapisz(string tytul, string? opis = null, string? maska = null, string? nazwa = null, string? katalog = null)
	{
		...
		if (!String.IsNullOrEmpty(katalog)) opts.SuggestedStartLocation = TUI.StorageProvider.TryGetFolderFromPathAsync(katalog).GetAwaiter().GetResult();
		...
		var plik = TUI.StorageProvider.SaveFilePickerAsync(opts).GetAwaiter().GetResult();
	...
	public static string? Katalog(string tytul)
	{
		...
		opts.Title = tytul;
		var katalogi = TUI.StorageProvider.OpenFolderPickerAsync(opts).GetAwaiter().GetResult();
