...
class FakturaRozliczenieZaliczkiAkcja : FakturaPodobnaAkcja
{
	...
	protected override Faktura? UtworzRekord(Kontekst kontekst, IEnumerable<Faktura> zaznaczoneRekordy)
	{
		...
		foreach (var zaliczka in zaznaczoneRekordy.OrderBy(faktura => faktura.DataWystawienia))
		{
			...
			var wplata = new Wplata { Data = zaliczka.DataWystawienia, FakturaRef = podobna, Kwota = zaliczka.RazemBrutto, Uwagi = Wplata.UwagiDlaZaliczki(zaliczka), CzyRozliczenie = true };
